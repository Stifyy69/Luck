...
const loginRateLimit = createRateLimiter({ windowMs: 15 * 60 * 1000, max: 12 });
const registerRateLimit = createRateLimiter({ windowMs: 15 * 60 * 1000, max: 12 });
const guestRateLimit = createRateLimiter({ windowMs: 60 * 1000, max: 30 });
...
app.post('/api/auth/register', registerRateLimit, requireDb, async (req, res) => {
  try {
...
app.post('/api/auth/guest', guestRateLimit, requireDb, async (req, res) => {
  try {
...
app.post('/api/auth/login', loginRateLimit, requireDb, async (req, res) => {
