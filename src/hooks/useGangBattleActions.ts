...
import { appendLog, wait } from '../lib/gangLocalState';
import { addMemberLoyalty, awardMemberActivity, type GangMember } from '../lib/gangMembers';
import { simulateGangBattle, toBattleHistoryEntry, type GangBotOpponent } from '../lib/gangBattles';
...
export function useGangBattleActions(o: Options) {
  const startBattle = (opponent: GangBotOpponent, ids: string[], leaderId: string) => {
    ...
    const result = simulateGangBattle(crew, leaderId, opponent);
    o.setBusy(true); o.setBattleOverlay({ opponentName: opponent.name, stageIndex: 0, progress: 0, stages: result.stages, result: null });
    void (async () => {
      try {
        ...
        o.setGangData((current) => {
          let members = awardMemberActivity(current.members, ids, { primary: 'shooting', secondary: 'tactics', leaderId, leaderPrimary: 'leadership', leaderSecondary: 'tactics', leaderXpBonus: 10 }, result.xpGain);
          ...
          return appendLog({ ...current, members, dirtyBalance: current.dirtyBalance + result.dirtyReward, dirtyEarned: current.dirtyEarned + result.dirtyReward, gunpowder: current.gunpowder + result.gunpowderReward, steel: current.steel + result.steelReward, battleReputation: current.battleReputation + result.reputationReward, battleHistory: [toBattleHistoryEntry(result, ids, leaderId), ...current.battleHistory].slice(0, 20), battleBoardSeed: current.battleBoardSeed + 1 }, `${result.won ? 'Battle won' : 'Battle lost'} vs ${opponent.name}, ${result.playerStageWins}-${result.opponentStageWins}.`, result.won ? 'positive' : 'negative');
        });
