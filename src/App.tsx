    ...
    <div className="relative min-h-screen">
      ...
      <div className={`hud-panel fixed inset-y-0 left-0 z-[60] w-[242px] rounded-r-2xl p-4 backdrop-blur-xl transition-transform md:left-5 md:top-4 md:h-[calc(100vh-2rem)] md:w-[226px] md:rounded-2xl ${menuOpen ? 'translate-x-0' : '-translate-x-[86%] md:translate-x-0'}`}>
        ...
        <button
          type="button"
          onClick={() => goTo('/pizzer')}
          className={`mt-2 block w-full rounded-xl px-4 py-3 text-left text-sm font-bold uppercase tracking-[0.06em] transition ${path === '/pizzer' ? 'btn-secondary shadow-[inset_3px_0_0_#ffb347]' : 'text-white/70 hover:bg-white/5'}`}
        >
          Pizzer
        </button>
        <div className="mt-4 border-t border-white/10 pt-4">
