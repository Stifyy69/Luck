...
function withoutGangEconomy(value: unknown) {
  if (!value || typeof value !== 'object') return value;
  const { gangData: _legacyGang, gangState: _gangState, ...genericState } = value as Record<string, unknown>;
  return genericState;
}
  ...
  const loadMe = async () => {
    ...
    if (stateResponse.ok) {
      ...
      if (statePayload?.state && Object.keys(statePayload.state).length > 0) {
        saveState(withoutGangEconomy(statePayload.state));
      }
    }
  };
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      ...
      fetch('/api/account/state', {
        ...
        credentials: 'include',
        body: JSON.stringify({ state: withoutGangEconomy(state) }),
      }).catch(() => {});
    }, 20_000);
