...
// ---------------------------------------------------------------------------
// Display rewards – used only for animation track visuals.
// Actual reward comes from the server after spin.
// ---------------------------------------------------------------------------
const rewards = [
  { name: 'Vortek 911R',      subtitle: 'Ultra Rare Jackpot', tier: 'legendary', emoji: '🏎️' },
  { name: 'Vortek Cayenne X', subtitle: 'Very Rare Jackpot',  tier: 'legendary', emoji: '🚗' },
  { name: 'VIP Gold',         subtitle: 'x2 bani 10 minute',  tier: 'epic',      emoji: '💎' },
  { name: 'VIP Silver',       subtitle: 'x2 bani 5 minute',   tier: 'epic',      emoji: '💠' },
  { name: 'Mystery Box',      subtitle: 'Haina random',        tier: 'epic',      emoji: '📦' },
  { name: 'Fragmente Ruleta', subtitle: 'x5 fragmente',        tier: 'rare',      emoji: '🪙' },
  { name: 'FlowCoins',        subtitle: '10 FC',               tier: 'rare',      emoji: '🟠' },
  { name: 'Slot Vehicle',     subtitle: '+1 slot vehicul',     tier: 'rare',      emoji: '➕' },
  { name: 'Voucher Showroom', subtitle: 'Discount showroom',   tier: 'rare',      emoji: '🎟️' },
  { name: 'Pilot Boost x2',   subtitle: 'O singura folosire',  tier: 'uncommon',  emoji: '✈️' },
  { name: 'Sleep Boost x2',   subtitle: 'O singura folosire',  tier: 'uncommon',  emoji: '😴' },
  { name: 'Scutire Taxe',     subtitle: 'Anuleaza urm. taxa',  tier: 'uncommon',  emoji: '💸' },
...
function pickLocalWeightedReward() {
  const total = rewards.reduce((s, r) => s + (tierWeight[r.tier] ?? 1), 0);
  let roll = Math.random() * total;
  for (const r of rewards) {
    roll -= tierWeight[r.tier] ?? 1;
    if (roll <= 0) return r;
  }
...
export default function RouletteDemo() {
  ...
  const [selectedReward, setSelectedReward] = useState<SpinResult | null>(null);
  const [showWinModal, setShowWinModal] = useState(false);
  const [highlightIndex, setHighlightIndex] = useState(START_INDEX);
  const [latestWins, setLatestWins] = useState([rewards[2], rewards[3], rewards[4], rewards[13]]);
  const [spinError, setSpinError] = useState<string | null>(null);

  // Player state – loaded from API (bootstrap) then kept in sync
  const [playerId] = useState(() => getPlayerId());
  const saved = typeof window !== 'undefined' ? loadGameState() : null;
  const [fragments, setFragments] = useState(Number(saved?.fragments ?? 0));
  const [flowCoinsBalance, setFlowCoinsBalance] = useState(Number(saved?.flowCoinsBalance ?? saved?.ogCoinsBalance ?? 0));
  const [bonusSpins, setBonusSpins] = useState(Number(saved?.bonusSpins ?? 0));
  const [cashBalance, setCashBalance] = useState(Number(saved?.cashBalance ?? 1_000_000));
  const [rouletteSpent, setRouletteSpent] = useState(Number(saved?.rouletteSpent ?? 0));
  const [rouletteWon, setRouletteWon] = useState(Number(saved?.rouletteWon ?? 0));
  const [timeFarm] = useState(Number(saved?.timeFarm ?? saved?.timeLostFarm ?? 0));
  const [timeSleep] = useState(Number(saved?.timeSleep ?? 0));
  const [processedFrunze] = useState(Number(saved?.processedFrunze ?? 0));
  const [processedWhite] = useState(Number(saved?.processedWhite ?? 0));
  const [processedBlue] = useState(Number(saved?.processedBlue ?? 0));
  const [farmEarned] = useState(Number(saved?.farmEarned ?? 0));
  const [timePilot] = useState(Number(saved?.timePilot ?? 0));
  ...
  useEffect(() => {
    api.bootstrap(playerId).then((player) => {
      setCashBalance(player.cleanMoney);
      setFlowCoinsBalance(player.flowCoins);
      setFragments(player.rouletteFragments);
      const extra = Math.floor(player.rouletteFragments / 4);
      setBonusSpins(extra);
    }).catch(() => {
  ...
  }, [fragments, flowCoinsBalance, bonusSpins, cashBalance, rouletteSpent, rouletteWon, timeFarm, timeSleep, processedFrunze, processedWhite, processedBlue, farmEarned, timePilot]);
  ...
  const playTone = ({ frequency = 440, sweepTo, duration = 0.08, volume = 0.02, type = 'triangle' }) => {
  ...
  const handleSpin = async (costType) => {
    ...
    if (costType === 'flowcoins' && flowCoinsBalance < 30 && bonusSpins < 1) return;
    ...
    // Cosmetic: pick local winner for animation only
    const cosmeticWinner = pickLocalWeightedReward();
    const cosmeticIndex = rewards.findIndex((r) => r.name === cosmeticWinner.name);
    const currentRewardIndex = currentIndexRef.current % rewards.length;
    const deltaToWinner = (cosmeticIndex - currentRewardIndex + rewards.length) % rewards.length;
    ...
    // Optimistically deduct cost from UI
    if (costType === 'cash') {
      setCashBalance((c) => c - 100_000);
      setRouletteSpent((c) => c + 100_000);
    } else if (bonusSpins > 0) {
      setBonusSpins((c) => c - 1);
    } else {
      setFlowCoinsBalance((c) => c - 30);
    }
    ...
    const spinPromise = api.rouletteSpin(playerId, costType).catch((err) => {
    ...
    scheduleTask(async () => {
      ...
      const serverResult: SpinResult | null = await spinPromise;

      if (!serverResult) {
        // API failed – revert optimistic deduction
        if (costType === 'cash') {
          setCashBalance((c) => c + 100_000);
          setRouletteSpent((c) => c - 100_000);
        } else if (bonusSpins > 0) {
        ...
        setSpinError('Spin-ul a esuat. Incearca din nou.');
        return;
      }

      // Update state from server
      setCashBalance(serverResult.player.cleanMoney);
      setFlowCoinsBalance(serverResult.player.flowCoins);
      setFragments(serverResult.player.rouletteFragments);
      setBonusSpins(Math.floor(serverResult.player.rouletteFragments / 4));
      if (serverResult.payout > 0) {
        setRouletteWon((c) => c + serverResult.payout);
      }

      // Find display reward matching server result
      const displayReward = rewards.find((r) => r.name === serverResult.rewardName) || {
        name: serverResult.rewardName,
        subtitle: serverResult.rewardSubtitle,
        tier: serverResult.tier,
        emoji: serverResult.emoji,
      };

      setSelectedReward(serverResult);
      setLatestWins((current) => [displayReward, ...current].slice(0, 5));
      playWinSound();
    }, SPIN_DURATION_MS + 40);
  ...
  useEffect(() => {
    const onKeyDown = (event) => {
      ...
      if (!isSpinning && cashBalance >= 100_000) handleSpin('cash');
    };
    window.addEventListener('keydown', onKeyDown);
    return () => window.removeEventListener('keydown', onKeyDown);
  }, [isSpinning, cashBalance]); // eslint-disable-line react-hooks/exhaustive-deps

  // Display reward for the win modal
  const displaySelectedReward = selectedReward
    ? rewards.find((r) => r.name === selectedReward.rewardName) || {
        name: selectedReward.rewardName,
        subtitle: selectedReward.rewardSubtitle,
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-[#130f2d] text-white">
        ...
        <div className="flex min-h-screen flex-col">
          ...
          <div className="mx-auto w-full max-w-[1120px] rounded-[20px] border border-white/10 bg-[#17143a]/72 px-2.5 py-2.5 shadow-[0_30px_100px_rgba(0,0,0,0.44)] backdrop-blur-xl sm:px-3 sm:py-3">
            ...
            <div className="mt-3 flex flex-col items-center justify-center gap-2 sm:flex-row sm:gap-2.5">
              ...
              <button
                type="button"
                onClick={() => handleSpin('flowcoins')}
                disabled={isSpinning || (flowCoinsBalance < 30 && bonusSpins < 1)}
              ...
              >
                {isSpinning && activeCost === 'flowcoins' ? 'Se deschide...' : 'Deschide cu 30 FC'}
              </button>
            </div>

            {spinError ? (
              <p className="mt-2 text-center text-xs text-red-400">{spinError}</p>
            ) : null}
            ...
            <div className="mt-2 grid grid-cols-1 gap-2 sm:grid-cols-3">
              <div className="rounded-[10px] border border-white/15 bg-black/25 px-3 py-2 text-center">
                ...
                <p className="text-lg font-black text-white">{fragments % 4}/4</p>
              </div>
              <div className="rounded-[10px] border border-white/15 bg-black/25 px-3 py-2 text-center">
                <p className="text-[10px] uppercase tracking-[0.2em] text-white/45">FlowCoins</p>
                <p className="text-lg font-black text-white">{flowCoinsBalance}/30</p>
              </div>
              <div className="rounded-[10px] border border-white/15 bg-black/25 px-3 py-2 text-center">
          ...
          <section className="mx-auto mt-7 w-full max-w-[1120px]">
            ...
            <div className="mt-2 grid grid-cols-2 gap-2.5 pb-14 sm:grid-cols-3 sm:gap-3 lg:grid-cols-7">
              {rewards.map((reward) => (
      ...
      {showWinModal && selectedReward && displaySelectedReward ? (
          ...
          <div className="w-full max-w-md rounded-[32px] border border-white/10 bg-[#17142f]/96 p-6 shadow-[0_24px_90px_rgba(0,0,0,0.5)]">
            ...
            <div className={`mt-5 rounded-[28px] border p-5 ${tierStyles[selectedReward.tier]}`}>
              <div className="mb-5 flex h-28 items-center justify-center rounded-[22px] bg-black/10 text-7xl backdrop-blur-sm">
                {displaySelectedReward.emoji}
              </div>
              <p className="text-xs uppercase tracking-[0.32em] text-white/45">{rarityLabel[selectedReward.tier]}</p>
              <h3 className="mt-2 text-3xl font-black leading-tight text-white">{selectedReward.rewardName}</h3>
              <p className="mt-2 text-base text-white/65">{selectedReward.rewardSubtitle}</p>
              {selectedReward.payout > 0 ? (
                <p className="mt-2 text-lg font-black text-emerald-300">+{selectedReward.payout.toLocaleString('ro-RO')} $</p>
              ) : null}
              {selectedReward.metadata?.discount ? (
                <p className="mt-1 text-sm text-amber-300">Discount: {String(selectedReward.metadata.discount)}%</p>
              ) : null}
            </div>
...
function RewardCard({ reward, styles, className = '', compact = false, highlighted = false, spinning = false }) {
  ...
  const highlightClasses = highlighted ? 'ring-2 ring-white/55 shadow-[0_0_38px_rgba(255,255,255,0.18)]' : '';
  return (
    <div className={`group relative overflow-hidden rounded-[10px] border p-2.5 transition duration-300 ${hoverClasses} ${styles[reward.tier]} ${highlightClasses} ${className}`}>
      ...
      <div className={`relative mb-2.5 flex items-center justify-center rounded-[9px] bg-black/15 backdrop-blur-sm ${compact ? 'h-[66px] text-4xl sm:h-[70px] sm:text-[2.45rem]' : 'h-[88px] text-5xl'}`}>
