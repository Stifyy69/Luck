...
export default function GangsPage({ section = 'overview' }: { section?: GangSection; onNavigate?: (path: string) => void }) {
  ...
  const injuredCount = gang.gangData.members.filter((member) => member.injuredUntilGameHour > gang.currentGameHour).length;
  ...
  const renderSection = () => {
    ...
    if (section === 'recruitment') {
      if (gang.isFull) return <section className="game-panel p-8 text-center"><p className="section-kicker">Recruitment</p><h1 className="mt-3 text-2xl font-black text-white">Roster full</h1><p className="mt-2 text-xs text-white/35">Dismiss a member before recruiting another candidate.</p></section>;
    ...
    return <GangMembersPanel members={gang.gangData.members} currentGameHour={gang.currentGameHour} recruitingPower={gang.recruitingPower} busy={gang.busy} onDismiss={gang.setDismissTarget} onSupport={gang.setSupportTarget} />;
  };
    ...
    <div className="min-h-screen bg-transparent px-4 pb-10 pt-24 text-white sm:px-6 md:px-8">
      ...
      <div className="mx-auto max-w-[1280px]">
        {!gang.formed ? (
          ...
          <>
            <GangContextBar section={section} name={gang.gangData.name} level={gang.level.name} memberCount={gang.gangData.members.length} maxMembers={gang.maxMembers} cleanBalance={gang.gangData.cleanBalance} dirtyBalance={gang.gangData.dirtyBalance} averageLoyalty={gang.averageLoyalty} stockValue={gang.stockValue} />
            {renderSection()}
          </>
        )}
      </div>
      <div className="mx-auto mt-5 max-w-[1280px]"><PageDisclaimer /></div>

      {gang.dismissTarget ? <div className="fixed inset-0 z-[134] flex items-center justify-center bg-black/75 p-4 backdrop-blur-sm" onClick={() => gang.setDismissTarget(null)}><div className="game-panel w-full max-w-md p-5" onClick={(event) => event.stopPropagation()}><p className="section-kicker">Dismiss member</p><h2 className="mt-3 text-2xl font-black text-white">Remove {gang.dismissTarget.displayName}?</h2><p className="mt-2 text-xs text-white/35">The remaining members lose loyalty.</p><div className="mt-5 grid grid-cols-2 gap-2"><button type="button" onClick={() => gang.setDismissTarget(null)} className="btn-ghost rounded-xl px-4 py-3 text-sm">Cancel</button><button type="button" onClick={gang.confirmDismiss} className="rounded-xl border border-red-300/20 bg-red-500/15 px-4 py-3 text-sm font-black text-red-100">Dismiss</button></div></div></div> : null}
